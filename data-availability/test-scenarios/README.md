...
> **Note on tooling:** The production evaluation was conducted using **Apache JMeter 5.5** with the exact traffic profile described in Section 3.5 (50 concurrent users, 5,000 requests/minute, 30-minute sustained run). The JMeter test plans are not included in this public package because they reference internal staging endpoints and credential placeholders that cannot be safely published. The **Artillery.io** scripts provided here are a functionally equivalent open-source reproducibility alternative, implementing identical scenarios, user flows, load phases, and performance thresholds against a configurable target URL. Reviewers wishing to validate the traffic profile can inspect the load phases in `payment-workflow.artillery.yml` and map them to the JMeter Thread Group configuration described in Section 3.3.
